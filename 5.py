...
class CapitalManager:
    ...
    def settle_daily(self,
        ...
        for item in buy_list:
            ...
            seed_v = sum(ord(c) for c in symbol)
            _rnd.seed(seed_v)
            intraday_low = pre_close * (1 + _rnd.uniform(stop_loss * 1.5, 0))
            intraday_high = pre_close * (1 + _rnd.uniform(0, stop_profit * 1.5))
            stop_triggered = False
            settle_reason = "收盘平仓"

            if intraday_low <= pre_close * (1 + stop_loss):
                today_close = pre_close * (1 + stop_loss)
                stop_triggered = True
                settle_reason = f"🔴 止损({stop_loss:.0%})"
                self.logger.debug(f"[止损] {symbol} {name}: 触发止损 {stop_loss:.0%}")
            elif intraday_high >= pre_close * (1 + stop_profit):
        ...
        # ---- 汇总 ----
        self.day_pnl = int(total_pnl)  # 转为整数（向下取整，保守处理）
    ...
    def get_capital_summary(self) -> str:
        ...
        lines.append(f"║  现金缓冲:      {int(self.available_cash - self._get_total_buy_budget() + self.day_total_buy_cost):>14,} 元       ║")
...
def run_daily_task(date_str: Optional[str] = None) -> Dict[str, Any]:
    ...
    capital_mgr.reset_first_day(initial_capital=INIT_CAPITAL)
    ...
    cap_summary = {
        "total_asset": capital_mgr.total_asset,
        "old_total_asset": capital_mgr.total_asset,
        "total_buy_cost": capital_mgr.day_total_buy_cost,
        "total_pnl": 0,  # 收盘结算前暂为0
        "total_pnl_pct": 0.0,
        "available_cash": capital_mgr.available_cash - capital_mgr.day_total_buy_cost,
        "settled_count": len(buy_list),
    }
    ...
    if buy_list:
        # 模拟当日收盘价（使用前收盘价×随机波动因子模拟）
        # 实际大赛场景中，此处应使用当日真实收盘价
        today_close_dict = {}
        for order in buy_list:
            symbol = order.get("symbol", "")
            pre_close = safe_float(order.get("pre_close", 0))
            # 模拟：当日收盘价 = 前收盘价 × (1 + 随机波动[-3%, +3%])
            # 使用symbol的hash作为确定性随机种子
            seed = sum(ord(c) for c in symbol) / 100.0
            simulated_return = (seed % 6.0 - 3.0) / 100.0  # [-3%, +3%]
            today_close_dict[symbol] = pre_close * (1 + simulated_return)
